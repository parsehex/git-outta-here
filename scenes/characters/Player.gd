...
func _ready():
	...
	pass
func _input(event):
	if event is InputEventKey and event.keycode == KEY_L and event.shift_pressed and event.pressed:
		var wood = Inventory.get_item('wood')
		if wood > 0:
			var sell_amount = wood * Globals.wood_price
			Inventory.add_money(sell_amount)
			Inventory.remove_item('wood', wood)
...
func get_facing_direction() -> Vector2:
	...
	return Vector2.DOWN # Default to down
